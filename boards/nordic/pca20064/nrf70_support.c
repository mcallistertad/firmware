...
int nrf_wifi_if_zep_stop_board(const struct device *dev)
{
	...
	/* regulator-boot-on owns one reference in addition to the scan request. */
	if (regulator_ret == 0 && (wifi_regulator_config->flags & REGULATOR_BOOT_ON) != 0 &&
	    !boot_power_disabled) {
		regulator_ret = regulator_disable(wifi_regulator);
